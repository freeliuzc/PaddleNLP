...
#pragma unroll
    ...
    if (!base_model_stop_flags[tid]) {
      ...
      } else if (accept_num_now <= max_draft_token)/*Accept partial draft tokens*/ {
...
void DraftModelPreprocess(
  ...
  auto not_need_stop_gpu = not_need_stop.copy_to(seq_lens_this_time.place(), false);


  if (draft_type == "eagle") {

    draft_model_preprocess_kernel<BlockSize, true><<<1, BlockSize, 0, cu_stream>>>(
    ...
    draft_model_preprocess_kernel<BlockSize, false><<<1, BlockSize, 0, cu_stream>>>(
      const_cast<int64_t*>(draft_tokens.data<int64_t>()),
      const_cast<int64_t*>(input_ids.data<int64_t>()),
      const_cast<bool*>(stop_flags.data<bool>()),
      const_cast<int*>(seq_lens_this_time.data<int>()),
