...
std::vector<paddle::Tensor> DispatchDtype(
  ...
  auto position_map = paddle::full({input_token_num}, -1, seq_lens_this_time.dtype(), input.place());
  ...
  auto out = paddle::full({output_token_num_cpu, dim_embed}, -1, input.dtype(), input.place());
