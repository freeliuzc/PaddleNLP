...
__global__ void update_pre_ids_kernel(
    ...
    if (tid < bs && seq_lens_this_time[tid] != 0 && !stop_flags[tid]) {
        ...
        if (step_idx[tid] -1 > 0 /*Decoder Step*/) {
            for (int i = 0; i < seq_len_this_time; ++i) {
                pre_ids_all_now[step_idx[tid] - i] = draft_token_now[seq_len_this_time - 1 - i];
            }
...
void SpeculateDraftModelUpdate(
...
PD_BUILD_OP(draft_model_set_value_by_flags)
  .Inputs({"draft_tokens", "pre_ids_all", "stop_flags", "seq_lens_this_time", "seq_lens_encoder", "seq_lens_decoder", "step_idx"})
