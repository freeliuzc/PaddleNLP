...
__global__ void draft_model_update_kernel(
  ...
  if (tid < bsz) {
    ...
    auto *base_model_draft_tokens_now = base_model_draft_tokens + tid * max_base_model_draft_token;
    const int next_tokens_start_id = tid * max_seq_len - output_cum_offsets[tid];
