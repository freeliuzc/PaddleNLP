...
class PredictorArgument:
    ...
    draft_model_name_or_path: str = field(
        default=None, metadata={"help": "The directory of eagle or draft model"}
    )
...
class BlockInferencePredictorMixin(BasePredictor):
    ...
    def _preprocess(self, input_text: list[str]):
        ...
        self.model_inputs["seq_lens_this_time"] = paddle.to_tensor(np.array(self.seq_lens).astype("int32").reshape(-1, 1))
        self.model_inputs["seq_lens_encoder"] = paddle.to_tensor(np.array(self.seq_lens).astype("int32").reshape(-1, 1))
...
class StaticGraphBlockInferencePredictor(BlockInferencePredictorMixin):
    ...
    def predict(self, input_texts: list[str], return_tokens=False):
        ...
        self._preprocess(input_texts)
        if self.proposer is not None:
            self.proposer.insert_query(base_model_inputs=self.model_inputs, real_bs=len(input_texts), seq_lens=self.seq_lens)
